...
public class GuiThemes {
    private static final File FOLDER = new File(MatHaxClientLegacy.VERSION_FOLDER, "GUI");
    ...
    public static void add(GuiTheme theme) {
        for (Iterator<GuiTheme> it = themes.iterator(); it.hasNext();) {
            if (it.next().name.equals(theme.name)) {
                ...
                MatHaxClientLegacy.LOG.error(MatHaxClientLegacy.logprefix + "Theme with the name '{}' has already been added.", theme.name);
                break;
            }
        }
