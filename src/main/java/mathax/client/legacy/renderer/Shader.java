...
public class Shader {
    ...
    public Shader(String vertPath, String fragPath) {
        ...
        if (vertError != null) {
            MatHaxClientLegacy.LOG.error(MatHaxClientLegacy.logprefix + "Failed to compile vertex shader (" + vertPath + "): " + vertError);
            throw new RuntimeException("[MatHax Legacy] Failed to compile vertex shader (" + vertPath + "): " + vertError);
        }
        ...
        if (fragError != null) {
            MatHaxClientLegacy.LOG.error(MatHaxClientLegacy.logprefix + "Failed to compile fragment shader (" + fragPath + "): " + fragError);
            throw new RuntimeException("[MatHax Legacy] Failed to compile fragment shader (" + fragPath + "): " + fragError);
        }
        ...
        if (programError != null) {
            MatHaxClientLegacy.LOG.error(MatHaxClientLegacy.logprefix + "Failed to link program: " + programError);
            throw new RuntimeException("[MatHax Legacy] Failed to link program: " + programError);
    ...
    private String read(String path) {
        try {
            return IOUtils.toString(mc.getResourceManager().getResource(new Identifier("mathaxlegacy", "shaders/" + path)).getInputStream(), StandardCharsets.UTF_8);
        } catch (IOException e) {
            e.printStackTrace();
            return "";
        }
