...
import mathax.client.legacy.systems.accounts.AccountType;
...
public class PremiumAccount extends Account<PremiumAccount> {
    ...
    public boolean login() {
        ...
        try {
            ...
            MatHaxClientLegacy.LOG.error(MatHaxClientLegacy.logprefix + "Failed to contact the authentication server.");
            return false;
        } catch (AuthenticationException e) {
            if (e.getMessage().contains("Invalid username or password") || e.getMessage().contains("account migrated"))
                MatHaxClientLegacy.LOG.error(MatHaxClientLegacy.logprefix + "Wrong password.");
            else MatHaxClientLegacy.LOG.error(MatHaxClientLegacy.logprefix + "Failed to contact the authentication server.");
            return false;
        }
    }
