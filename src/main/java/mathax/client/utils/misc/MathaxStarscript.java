...
public class MathaxStarscript {
    ...
    public static void init() {
        ...
        ss.set("ping", MathaxStarscript::ping);
        ss.set("time", () -> Value.string(LocalTime.now().format(DateTimeFormatter.ofLocalizedTime(FormatStyle.SHORT))));

        // Meteor
        ss.set("mathax", new ValueMap()
            .set("name", MatHax.NAME)
            .set("version", Version.getStylized())
            .set("modules", () -> Value.number(Modules.get().getAll().size()))
            .set("active_modules", () -> Value.number(Modules.get().getActive().size()))
            .set("is_module_active", MathaxStarscript::isModuleActive)
            .set("get_module_info", MathaxStarscript::getModuleInfo)
            .set("prefix", MathaxStarscript::getMeteorPrefix)
        );
        ...
        ss.set("baritone", new ValueMap()
            ...
            .set("distance_to_goal", MathaxStarscript::baritoneDistanceToGoal)
            .set("process", MathaxStarscript::baritoneProcess)
            .set("process_name", MathaxStarscript::baritoneProcessName)
            .set("eta", MathaxStarscript::baritoneETA)
        );
        ...
        ss.set("player", new ValueMap()
            ...
            .set("biome", MathaxStarscript::biome)
            ...
            .set("hand_or_offhand", MathaxStarscript::handOrOffhand)
            .set("get_item", MathaxStarscript::getItem)
            .set("count_items", MathaxStarscript::countItems)
            ...
            .set("has_potion_effect", MathaxStarscript::hasPotionEffect)
            .set("get_potion_effect", MathaxStarscript::getPotionEffect)

            .set("get_stat", MathaxStarscript::getStat)
        );

        // Crosshair target
        ss.set("crosshair_target", new ValueMap()
            .set("type", MathaxStarscript::crosshairType)
            .set("value", MathaxStarscript::crosshairValue)
        );
    ...
    private static String getCallerName() {
        ...
        for (int i = 1; i < elements.length; i++) {
            ...
            if (name.equals(MathaxStarscript.class.getName())) continue;
