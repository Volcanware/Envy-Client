...
public class ScaffoldPlus extends Module {
    ...
    public ScaffoldPlus() {
        super(Categories.Movement, Items.SCAFFOLDING, "ScaffoldPlus", "Scaffolds blocks under you.");
    }
    private int slot = -1;
    ...
    public void tick(TickEvent.Pre event) {
        ...
        for (int i = 0; i <= (mc.player.getVelocity().x == 0.0 && mc.player.getVelocity().z == 0.0 ? 0 : ext.get()); i++) {
            ...
            if (tower.get() && mc.options.jumpKey.isPressed() && mc.player.getVelocity().x == 0.0 && mc.player.getVelocity().z == 0.0) {
                if (mc.world.getBlockState(mc.player.getBlockPos().down()).canPlaceAt(mc.world, mc.player.getBlockPos().down()) &&
                    !mc.world.getBlockState(mc.player.getBlockPos().down(2)).canPlaceAt(mc.world, mc.player.getBlockPos().down(2)) &&
