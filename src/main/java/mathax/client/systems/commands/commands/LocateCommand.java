...
public class LocateCommand extends Command {
    ...
    private Vec3d findByBlockList(List<Block> blockList) {
        ...
        if (posList.size() < 3) warning("Only %d block(s) found. This search might be a false positive.", posList.size());
    ...
    private void onReadPacket(PacketEvent.Receive event) {
        if (event.packet instanceof EntitySpawnS2CPacket) {
            ...
            if (packet.getEntityTypeId() == EntityType.EYE_OF_ENDER) firstPosition(packet.getX(),packet.getY(),packet.getZ());
        }

        if (event.packet instanceof PlaySoundS2CPacket) {
            PlaySoundS2CPacket packet = (PlaySoundS2CPacket) event.packet;
            if (packet.getSound() == SoundEvents.ENTITY_ENDER_EYE_DEATH) lastPosition(packet.getX(), packet.getY(), packet.getZ());
        }
    }

    private void firstPosition(double x, double y, double z) {
        Vec3d pos = new Vec3d(x, y, z);
        if (firstStart == null) firstStart = pos;
        else secondStart = pos;
    }
