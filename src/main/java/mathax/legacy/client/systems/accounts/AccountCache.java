...
public class AccountCache implements ISerializable<AccountCache> {
    ...
    public boolean loadHead(String url) {
        try {
            ...
            MatHaxLegacy.LOG.error(MatHaxLegacy.logPrefix + "Failed to read skin url (" + url + ").");
            return false;
        }
    }
    ...
    public static void loadSteveHead() {
        try {
            ByteBuffer data = TextureUtil.readResource(mc.getResourceManager().getResource(new Identifier("mathaxlegacy", "textures/steve.png")).getInputStream());
            data.rewind();
