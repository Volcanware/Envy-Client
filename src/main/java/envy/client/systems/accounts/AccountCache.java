...
public class AccountCache implements ISerializable<AccountCache> {
    ...
    public boolean loadHead(String url) {
        try {
            ...
            Envy.LOG.error("Failed to read skin url (" + url + ").");
            return false;
        }
    }
    ...
    public static void loadSteveHead() {
        try {
            ByteBuffer data = TextureUtil.readResource(Envy.mc.getResourceManager().getResource(new Identifier("envy", "textures/steve.png")).get().getInputStream());
            data.rewind();
