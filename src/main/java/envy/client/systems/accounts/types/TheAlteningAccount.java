...
import envy.client.systems.accounts.AccountUtils;
...
public class TheAlteningAccount extends Account<TheAlteningAccount> {
    ...
    public boolean login() {
        ...
        try {
            ...
            Envy.LOG.error("Failed to login with The Altening.");
            return false;
        }
    }

    private YggdrasilUserAuthentication getAuth() {
        YggdrasilUserAuthentication auth = (YggdrasilUserAuthentication) new YggdrasilAuthenticationService(((MinecraftClientAccessor) Envy.mc).getProxy(), "", Environment.create(AUTH, ACCOUNT, SESSION, SERVICES, "The Altening")).createUserAuthentication(Agent.MINECRAFT);

        auth.setUsername(name);
        auth.setPassword("Envy on top!");
