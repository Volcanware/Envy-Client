...
import envy.client.systems.commands.arguments.CompoundNbtTagArgumentType;
        ...
        builder.then(literal("add").then(argument("nbt_data", CompoundNbtTagArgumentType.nbtTag()).executes(s -> {
            ...
            if (validBasic(stack)) {
                ...
                if (tag != null) {
                    source.copyFrom(tag);
                    setStack(stack);
                } else error("Some of the NBT data could not be found, try using: " + Config.get().prefix.get() + "nbt set {nbt}");
            }
