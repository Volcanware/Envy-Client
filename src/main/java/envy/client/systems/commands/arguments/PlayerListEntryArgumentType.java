...
public class PlayerListEntryArgumentType implements ArgumentType<PlayerListEntry> {
    ...
    static {
        if (Envy.mc.getNetworkHandler() != null) {
            EXAMPLES = Envy.mc.getNetworkHandler().getPlayerList()
                .stream()
                .limit(3)
    ...
    public PlayerListEntry parse(StringReader reader) throws CommandSyntaxException {
        ...
        for (PlayerListEntry p : Envy.mc.getNetworkHandler().getPlayerList()) {
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        return CommandSource.suggestMatching(Envy.mc.getNetworkHandler().getPlayerList().stream().map(playerListEntry -> playerListEntry.getProfile().getName()), builder);
    }
