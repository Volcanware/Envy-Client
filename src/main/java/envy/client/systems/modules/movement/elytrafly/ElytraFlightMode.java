...
public class ElytraFlightMode {
    ...
    public void onTick() {
        ...
        if (elytraFly.replace.get()) {
            ItemStack chestStack = Envy.mc.player.getInventory().getArmorStack(2);
    ...
    public void autoTakeoff() {
        ...
        boolean jumpPressed = Envy.mc.options.jumpKey.isPressed();

        if (elytraFly.autoTakeOff.get() && jumpPressed) {
            if (!lastJumpPressed && !Envy.mc.player.isFallFlying()) {
                jumpTimer = 0;
            ...
            if (jumpTimer >= 8) {
                ...
                Envy.mc.player.setJumping(false);
                Envy.mc.player.setSprinting(true);
                Envy.mc.player.jump();
                Envy.mc.getNetworkHandler().sendPacket(new ClientCommandC2SPacket(Envy.mc.player, ClientCommandC2SPacket.Mode.START_FALL_FLYING));
            }
        }
    ...
    public void handleAutopilot() {
        if (!Envy.mc.player.isFallFlying()) return;

        if (elytraFly.autoPilot.get() && Envy.mc.player.getY() > elytraFly.autoPilotMinimumHeight.get()) {
            Envy.mc.options.forwardKey.setPressed(true);
        ...
        if (elytraFly.useFireworks.get()) {
            if (ticksLeft <= 0) {
                ...
                if (itemResult.isOffhand()) {
                    Envy.mc.interactionManager.interactItem(Envy.mc.player, Hand.OFF_HAND);
                    Envy.mc.player.swingHand(Hand.OFF_HAND);
                } else {
                    InvUtils.swap(itemResult.slot(), true);

                    Envy.mc.interactionManager.interactItem(Envy.mc.player, Hand.MAIN_HAND);
    ...
    public void handleVerticalSpeed(PlayerMoveEvent event) {
        if (Envy.mc.options.jumpKey.isPressed()) velY += 0.5 * elytraFly.verticalSpeed.get();
        else if (Envy.mc.options.sneakKey.isPressed()) velY -= 0.5 * elytraFly.verticalSpeed.get();
    }
