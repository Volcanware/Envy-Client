...
import static envy.client.utils.player.PlayerUtils.directionSpeed;
...
public class RubberbandFly extends Module {
    ...
    public RubberbandFly() {
        super(Categories.Movement, Items.ACACIA_BUTTON, "rubberband-fly", "Fly with rubberbanding.");
    }
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        if (move && isPlayerMoving()) {
            ...
            switch (horizontalMode.get()) {
                case Clip -> {
                    double[] distance = directionSpeed((float) ((double) Math.round((speed.get() / 16.666) * 1000) / 1000));
