package envy.client.systems.modules;
...
import envy.client.events.mathax.ActiveModulesChangedEvent;
...
import envy.client.systems.modules.chat.*;
import envy.client.systems.modules.client.*;
import envy.client.systems.modules.client.swarm.Swarm;
import envy.client.systems.modules.combat.*;
import envy.client.systems.modules.experimental.*;
import envy.client.systems.modules.ghost.*;
import envy.client.systems.modules.misc.*;
import envy.client.systems.modules.movement.*;
import envy.client.systems.modules.movement.elytrafly.ElytraFly;
import envy.client.systems.modules.movement.speed.Speed;
import envy.client.systems.modules.player.*;
import envy.client.systems.modules.render.*;
import envy.client.systems.modules.render.marker.Marker;
import envy.client.systems.modules.render.search.Search;
import envy.client.systems.modules.world.Timer;
import envy.client.systems.modules.world.*;
import envy.client.utils.Utils;
import envy.client.utils.misc.ChatUtils;
import envy.client.utils.misc.ValueComparableMap;
...
public class Modules extends System<Modules> {
    ...
    void addActive(Module module) {
        synchronized (active) {
            if (!active.contains(module)) {
                active.add(module);
                Envy.EVENT_BUS.post(ActiveModulesChangedEvent.get());
            }
        }
    }
    ...
    void removeActive(Module module) {
        synchronized (active) {
            if (active.remove(module)) Envy.EVENT_BUS.post(ActiveModulesChangedEvent.get());
        }
    }
    ...
    private boolean onBinding(boolean isKey, int value) {
        if (moduleToBind != null && moduleToBind.keybind.canBindTo(isKey, value)) {
            if (value != GLFW.GLFW_KEY_ESCAPE) {
                ...
                Envy.mc.getToastManager().add(new ToastSystem(moduleToBind.icon, moduleToBind.category.color, moduleToBind.title, null, Formatting.GRAY + "Bound to " + Formatting.WHITE + moduleToBind.keybind + Formatting.GRAY + ".", Config.get().toastDuration.get()));
            }

            Envy.EVENT_BUS.post(ModuleBindChangedEvent.get(moduleToBind));
            moduleToBind = null;
            return true;
        }
    ...
    private void onAction(boolean isKey, int value, boolean isPress) {
        if (Envy.mc.currentScreen == null && !Input.isKeyPressed(GLFW.GLFW_KEY_F3)) {
            for (Module module : moduleInstances.values()) {
        ...
        synchronized (active) {
            for (Module module : modules) {
                if (module.isActive() && !module.runInMainMenu) {
                    Envy.EVENT_BUS.subscribe(module);
                    module.onActivate();
                }
            }
        }
    ...
    private void onGameLeft(GameLeftEvent event) {
        synchronized (active) {
            for (Module module : modules) {
                if (module.isActive() && !module.runInMainMenu) {
                    Envy.EVENT_BUS.unsubscribe(module);
                    module.onDeactivate();
                }
            }
        }
...
//        add(new AutoGrind());
    ...
    public static class ModuleRegistry extends Registry<Module> {
        public ModuleRegistry() {
            super(RegistryKey.ofRegistry(new Identifier("envy", "modules")), Lifecycle.stable());
        }
