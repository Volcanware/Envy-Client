...
import envy.client.eventbus.listeners.ConsumerListener;
import envy.client.events.game.WindowResizedEvent;
import envy.client.events.render.RenderAfterWorldEvent;
import envy.client.gui.WidgetScreen;
import envy.client.renderer.Framebuffer;
...
public class Background extends Module {
    ...
    public Background() {
        super(Categories.Render, Items.TINTED_GLASS, "Blur", "Adds Blur Effecti when in GUI screens");

        // The listeners need to run even when the module is not enabled
        Envy.EVENT_BUS.subscribe(new ConsumerListener<>(WindowResizedEvent.class, event -> {
            if (fbo1 != null) {
                fbo1.resize();
                fbo2.resize();
            }
        ...
        Envy.EVENT_BUS.subscribe(new ConsumerListener<>(RenderAfterWorldEvent.class, event -> onRenderAfterWorld()));
    }
