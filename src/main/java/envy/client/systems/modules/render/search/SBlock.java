...
public class SBlock {
    ...
    public void update() {
        state = Envy.mc.world.getBlockState(blockPos.set(x, y, z));
        neighbours = 0;
    ...
    private boolean isNeighbour(Direction dir) {
        ...
        BlockState neighbourState = Envy.mc.world.getBlockState(blockPos);

        if (neighbourState.getBlock() != state.getBlock()) return false;

        VoxelShape cube = VoxelShapes.fullCube();
        VoxelShape shape = state.getOutlineShape(Envy.mc.world, blockPos);
        VoxelShape neighbourShape = neighbourState.getOutlineShape(Envy.mc.world, blockPos);
    ...
    private boolean isNeighbourDiagonal(double x, double y, double z) {
        ...
        return state.getBlock() == Envy.mc.world.getBlockState(blockPos).getBlock();
    }
