...
import envy.client.utils.algorithms.RandomUtils;
import envy.client.utils.algorithms.Smooth;
import envy.client.utils.algorithms.extra.Perlin2D;
import envy.client.utils.algorithms.extra.RaycastUtils;
...
public class KillAuraPlus extends Module {
    ...
    public KillAuraPlus() {
        super(Categories.Combat, Items.DIAMOND_SWORD, "kill-aura-plus", "Smack's people without the anticheat noticing you.");
    }
	...
	private final Setting<RotationRandomize> rotationRandomize = sgGeneral.add(new EnumSetting.Builder<RotationRandomize>()
		.name("rotation-randomize")
		.description("Rotation randomize.")
		.defaultValue(RotationRandomize.None)
	...
	private final Setting<Integer> rotationRandomizeMultiply = sgGeneral.add(new IntSetting.Builder()
		...
		.range(0, 32)
		.sliderRange(0, 32)
		.visible(() -> rotationRandomize.get() != RotationRandomize.None && rotationSmooth.isVisible())
		.build()
	);
	...
	private RotationUtils.Rotation getRotate(Entity target) {
		...
		if ((result == null || result.getEntity() == null || result.getEntity() != target) && rayTraceRotate.get()) {
			...
			if (rotationRandomize.get() == RotationRandomize.Perlin) {
			...
			else if (rotationRandomize.get() == RotationRandomize.Random) {
			...
			else if (rotationRandomize.get() == RotationRandomize.RandomPerlin) {
