...
public class Shader {
    ...
    public Shader(String vertPath, String fragPath) {
        ...
        if (vertError != null) {
            Envy.LOG.error("Failed to compile vertex shader (" + vertPath + "): " + vertError);
            throw new RuntimeException("[Envy] Failed to compile vertex shader (" + vertPath + "): " + vertError);
        }
        ...
        if (fragError != null) {
            Envy.LOG.error("Failed to compile fragment shader (" + fragPath + "): " + fragError);
            throw new RuntimeException("[Envy] Failed to compile fragment shader (" + fragPath + "): " + fragError);
        }
        ...
        if (programError != null) {
            Envy.LOG.error("Failed to link program: " + programError);
            throw new RuntimeException("[Envy] Failed to link program: " + programError);
    ...
    private String read(String path) {
        try {
            return IOUtils.toString(mc.getResourceManager().getResource(new Identifier("envy", "shaders/" + path)).get().getInputStream(), StandardCharsets.UTF_8);
