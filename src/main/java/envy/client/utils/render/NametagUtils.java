...
import envy.client.utils.misc.Vec4;
...
public class NametagUtils {
    ...
    public static void onRender(MatrixStack matrices, Matrix4f projection) {
        ...
        camera.set(Envy.mc.gameRenderer.getCamera().getPos());
        cameraNegated.set(camera);
        cameraNegated.negate();

        windowScale = Envy.mc.getWindow().calculateScaleFactor(1, Envy.mc.forcesUnicodeFont());
    }
    ...
    public static boolean to2D(Vec3 pos, double scale) {
        ...
        pmMat4.toScreen();
        double x = pmMat4.x * Envy.mc.getWindow().getFramebufferWidth();
        double y = pmMat4.y * Envy.mc.getWindow().getFramebufferHeight();

        if (Double.isInfinite(x) || Double.isInfinite(y)) return false;

        pos.set(x / windowScale, Envy.mc.getWindow().getFramebufferHeight() - y / windowScale, pmMat4.z);
        return true;
    }
