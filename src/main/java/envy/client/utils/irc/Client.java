...
public class Client {
    ...
    static {
        try {
            File file = FabricLoader.getInstance().getConfigDir().resolve("envy").resolve("irc.json").toFile();
            if (file.exists()) {
    ...
    public static void sendToChat(Text text) {
        Envy.mc.inGameHud.getChatHud().addMessage(prefix.copy().append(text));
    }
