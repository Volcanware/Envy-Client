...
public class RotationUtils {
	...
	{
		ClientPlayerEntity player = Envy.mc.player;
	...
	public static Rotation toRotation(final Vec3d vec, final boolean predict) {
		final Vec3d eyesPos = new Vec3d(Envy.mc.player.getX(), Envy.mc.player.getBoundingBox().minY +
			Envy.mc.player.getEyeHeight(Envy.mc.player.getPose()), Envy.mc.player.getZ());

		if(predict) {
			if(Envy.mc.player.isOnGround()) {
				eyesPos.add(Envy.mc.player.getVelocity().x, 0.0, Envy.mc.player.getVelocity().z);
			}else eyesPos.add(Envy.mc.player.getVelocity().x, Envy.mc.player.getVelocity().y, Envy.mc.player.getVelocity().z);
		}
