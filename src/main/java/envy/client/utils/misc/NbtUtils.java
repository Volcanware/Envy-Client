...
public class NbtUtils {
    ...
    public static boolean toClipboard(String name, NbtCompound nbtCompound) {
        String preClipboard = Envy.mc.keyboard.getClipboard();
        try {
            ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
            NbtIo.writeCompressed(nbtCompound, byteArrayOutputStream);
            Envy.mc.keyboard.setClipboard(Base64.getEncoder().encodeToString(byteArrayOutputStream.toByteArray()));
            return true;
        } catch (Exception e) {
            Envy.LOG.error(String.format("Error copying %s NBT to clipboard!", name));
            ...
            Envy.mc.keyboard.setClipboard(preClipboard);
            return false;
        }
    }
    ...
    public static NbtCompound fromClipboard(NbtCompound schema) {
        try {
            byte[] data = Base64.getDecoder().decode(Envy.mc.keyboard.getClipboard().trim());
            ...
            Envy.LOG.error("Invalid NBT data pasted!");
