...
public class CombatHelper {
    ...
    public static BlockPos getCityBlock(PlayerEntity target, boolean legacyMode) {
        if (target == null || Envy.mc.player == null) return null;
    ...
    public static ArrayList<Hole> getHoles(int radius, int height) {
        List<BlockPos> nearby = BlockHelper.getSphere(Envy.mc.player.getBlockPos(), radius, height);
    ...
    public static boolean isValidHole(BlockPos holePos) {
        if (Envy.mc.player.getBlockPos().equals(holePos) /* ||BlockHelper.isWeb(holePos) */) return false;
    ...
    public static void mine(BlockPos pos, boolean rotate, int priority) {
        if (pos == null) return;
        if (!(Envy.mc.player.getMainHandStack().getItem() instanceof PickaxeItem)) {
