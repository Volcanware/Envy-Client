...
public class Interactions {
    ...
    public static class MineInstance {
        ...
        public void tick() {
            FindItemResult pick = findPick();
            if (!pick.found()) return;
            this.progress += getBreakDelta(pick.slot(), Envy.mc.world.getBlockState(this.pos));
        }
    ...
    public static double getBlockBreakingSpeed(int slot, BlockState block) {
        double speed = Envy.mc.player.getInventory().main.get(slot).getMiningSpeedMultiplier(block);

        if (speed > 1) {
            ItemStack tool = Envy.mc.player.getInventory().getStack(slot);
            int eff = EnchantmentHelper.getLevel(Enchantments.EFFICIENCY, tool);
            if (eff > 0 && !tool.isEmpty()) speed += eff * eff + 1;
        }

        if (StatusEffectUtil.hasHaste(Envy.mc.player)) speed *= 1 + (StatusEffectUtil.getHasteAmplifier(Envy.mc.player) + 1) * 0.2F;

        if (Envy.mc.player.hasStatusEffect(StatusEffects.MINING_FATIGUE)) {
            float k = switch (Envy.mc.player.getStatusEffect(StatusEffects.MINING_FATIGUE).getAmplifier()) {
                case 0 -> 0.3F;
                case 1 -> 0.09F;
                case 2 -> 0.0027F;
                case 3 -> 8.1E-4F;
        ...
        if (Envy.mc.player.isSubmergedIn(FluidTags.WATER) && !EnchantmentHelper.hasAquaAffinity(Envy.mc.player)) speed /= 5.0F;
        if (!Envy.mc.player.isOnGround()) speed /= 5.0F;
        return speed;
    }
    ...
    public static double getBreakDelta(int slot, BlockState state) {
        ...
        if (hardness == -1) return 0;
            ...
            return getBlockBreakingSpeed(slot, state) / hardness / (!state.isToolRequired() || Envy.mc.player.getInventory().main.get(slot).isSuitableFor(state) ? 30 : 100);
        }
    }

    public static void mine(BlockPos pos) {
        Envy.mc.interactionManager.updateBlockBreakingProgress(pos, Direction.UP);
    ...
    public static void setHVelocity(double x, double z) {
        Envy.mc.player.setVelocity(x, Envy.mc.player.getVelocity().getY(), z);
    }

    public static void setYVelocity(double y) {
        Vec3d velocity = Envy.mc.player.getVelocity();
        Envy.mc.player.setVelocity(velocity.x, y, velocity.z);
    }


    // Misc item methods
    public static int getSlot() {
        return Envy.mc.player.getInventory().selectedSlot;
    }
    public static Item getMainHandItem() { return Envy.mc.player.getInventory().getMainHandStack().getItem(); }
    ...
    public static boolean isHoldingBed() {return Envy.mc.player.getMainHandStack().getItem() instanceof BedItem;}

    // "Proxy" methods to check CombatHelper stuff on mc.player
    public static boolean isInHole() {return CombatHelper.isInHole(Envy.mc.player);}
    public static boolean isBurrowed() {return CombatHelper.isBurrowed(Envy.mc.player);}
    public static boolean isCitied() {return CombatHelper.isCitied(Envy.mc.player);}
    public static boolean isWebbed() {return CombatHelper.isWebbed(Envy.mc.player);}
    public static boolean isMoving() {return CombatHelper.isMoving(Envy.mc.player);}
    public static boolean isSelfTrapped() {return CombatHelper.isSelfTrapped(Envy.mc.player);}
    public static boolean isTopTrapped() {return CombatHelper.isTopTrapped(Envy.mc.player);}
    ...
    public static void setSlot(int slot, boolean packet) {
        if (slot < 0) return;
        lastSlot = Envy.mc.player.getInventory().selectedSlot;
        if (packet) {
    ...
    public static void windowClick(ScreenHandler handler, int slot, SlotActionType action, int clickData) {
        if (Envy.mc.player == null || Envy.mc.world == null || Envy.mc.interactionManager == null) return;
        Envy.mc.interactionManager.clickSlot(handler.syncId, slot, clickData, action, Envy.mc.player);
    }

    public static boolean isCrafting() {
        return Envy.mc.player.currentScreenHandler instanceof CraftingScreenHandler;
    }
    ...
    public static ItemStack getStackFromSlot(Integer slot) {
        ...
        return Envy.mc.player.getInventory().getStack(slot);
    }

    public static Item getItemFromSlot(Integer slot) {
        if (slot == -1) return null;
        if (slot == 45) return Envy.mc.player.getOffHandStack().getItem();
        return Envy.mc.player.getInventory().getStack(slot).getItem();
    }
    ...
    public static ItemStack getArmor(int slot) {return Envy.mc.player.getInventory().armor.get(slot);}
    public static Item getArmorItem(int slot) {return getArmor(slot).getItem();}

    public static boolean isInElytra() {return getArmorItem(2) == Items.ELYTRA && Envy.mc.player.isFallFlying();}
    ...
    public static PlayerEntity getPlayerByUUID(String uuid) {
        return Envy.mc.world.getPlayerByUuid(UUID.fromString(uuid));
    }

    public static PlayerEntity getPlayerByName(String name) {
        PlayerEntity p = null;
        for (PlayerEntity entity : Envy.mc.world.getPlayers()) {
    ...
    public static String getCurrentIGN() {
        return Envy.mc.getSession().getUsername();
    }
    ...
    public boolean doesPlayerOwn(Entity entity) {
        return doesPlayerOwn(entity, Envy.mc.player);
    }
    ...
    public static boolean isTeamed(PlayerEntity player1, PlayerEntity player2, boolean checkArmor) {
        ...
        if (checkArmor) {
            ItemStack p1armor = Envy.mc.player.getInventory().getArmorStack(3);
