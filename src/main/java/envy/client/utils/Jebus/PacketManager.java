...
public class PacketManager {
    ...
    public static void sendPacket(Packet packet) {
        ...
        Objects.requireNonNull(Envy.mc.getNetworkHandler()).sendPacket(packet);
    }
    ...
    public static void clipUp(int i) {
        if (i == -1) return;
        sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(Envy.mc.player.getX(), Envy.mc.player.getY() + i, Envy.mc.player.getZ(), false));
    }
    ...
    public static void rotate(BlockPos pos, Runnable task) {
        if (pos == null) return;
        sendPacket(new PlayerMoveC2SPacket.LookAndOnGround((float) Rotations.getYaw(pos), (float) Rotations.getPitch(pos), Envy.mc.player.isOnGround()));
        task.run();
    }
    ...
    public static void sendInteract(Hand hand, FindItemResult item, BlockHitResult hitResult, boolean rotate, boolean packet) {
        ...
        if (rotate) {
            if (CombatHelper.isInHole(Envy.mc.player)) rotate(pos, () -> sendInteract(hand, hitResult, packet));
    ...
    public static void sendInteract(Hand hand, BlockHitResult result, boolean packet) {
        ...
        if (packet) { // "packet placing" (sending interaction + swing packet directly)
            ...
            Envy.mc.interactionManager.interactBlock(Envy.mc.player, hand, result);
            Envy.mc.player.swingHand(hand);
        }
    }

    public static void sendBurrow() {
        sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(Envy.mc.player.getX(), Envy.mc.player.getY() + 0.4, Envy.mc.player.getZ(), true));
        sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(Envy.mc.player.getX(), Envy.mc.player.getY() + 0.75, Envy.mc.player.getZ(), true));
        sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(Envy.mc.player.getX(), Envy.mc.player.getY() + 1.01, Envy.mc.player.getZ(), true));
        sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(Envy.mc.player.getX(), Envy.mc.player.getY() + 1.15, Envy.mc.player.getZ(), true));
    }
