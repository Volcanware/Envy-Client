...
import envy.client.mixin.CrossbowItemAccessor;
import envy.client.mixin.ProjectileInGroundAccessor;
import envy.client.mixininterface.IVec3d;
import envy.client.utils.Utils;
import envy.client.utils.misc.MissHitResult;
...
public class ProjectileEntitySimulator {
    ...
    public boolean set(Entity user, ItemStack itemStack, double simulated, boolean accurate, double tickDelta) {
        ...
        if (item instanceof BowItem) {
            double charge = BowItem.getPullProgress(Envy.mc.player.getItemUseTime());
    ...
    public HitResult tick() {
        ...
        if (pos.y < Envy.mc.world.getBottomY()) return MissHitResult.INSTANCE;

        // Check if chunk is loaded
        int chunkX = (int) (pos.x / 16);
        int chunkZ = (int) (pos.z / 16);
        if (!Envy.mc.world.getChunkManager().isChunkLoaded(chunkX, chunkZ)) return MissHitResult.INSTANCE;
    ...
    private boolean isTouchingWater() {
        ...
        FluidState fluidState = Envy.mc.world.getFluidState(blockPos);
    ...
    private HitResult getCollision() {
        ...
        HitResult hitResult = Envy.mc.world.raycast(new RaycastContext(vec3d3, pos3d, RaycastContext.ShapeType.COLLIDER, waterDrag == 0 ? RaycastContext.FluidHandling.ANY : RaycastContext.FluidHandling.NONE, Envy.mc.player));
        if (hitResult.getType() != HitResult.Type.MISS) vec3d3 = hitResult.getPos();

        HitResult hitResult2 = ProjectileUtil.getEntityCollision(Envy.mc.world, Envy.mc.player, vec3d3, pos3d, new Box(pos.x, pos.y, pos.z, pos.x, pos.y, pos.z).stretch(Envy.mc.player.getVelocity()).expand(1.0D), entity -> !entity.isSpectator() && entity.isAlive() && entity.collides());
