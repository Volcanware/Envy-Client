package envy.client.utils.entity;

import envy.client.Envy;
import envy.client.mixin.EntityTrackingSectionAccessor;
import envy.client.mixin.SectionedEntityCacheAccessor;
import envy.client.mixin.SimpleEntityLookupAccessor;
import envy.client.mixin.WorldAccessor;
...
public class EntityUtils {
    ...
    public static int getPing(PlayerEntity player) {
        ...
        PlayerListEntry playerListEntry = Envy.mc.getNetworkHandler().getPlayerListEntry(player.getUuid());
    ...
    public static boolean isAboveWater(Entity entity) {
        ...
        for (int i = 0; i < 64; i++) {
            BlockState state = Envy.mc.world.getBlockState(blockPos);
    ...
    public static boolean isInRenderDistance(double posX, double posZ) {
        double x = Math.abs(Envy.mc.gameRenderer.getCamera().getPos().x - posX);
        double z = Math.abs(Envy.mc.gameRenderer.getCamera().getPos().z - posZ);
        double d = (Envy.mc.options.getViewDistance().getValue() + 1) * 16;
    ...
    public static List<BlockPos> getSurroundBlocks(PlayerEntity player) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (Envy.mc.world.getBlockState(pos).getBlock() == Blocks.OBSIDIAN) positions.add(pos);
        }
    ...
    public static boolean intersectsWithEntity(Box box, Predicate<Entity> predicate) {
        EntityLookup<Entity> entityLookup = ((WorldAccessor) Envy.mc.world).getEntityLookup();
    ...
    public static boolean isGreenHole(LivingEntity target) {
        assert Envy.mc.world != null;
        return Envy.mc.world.getBlockState(target.getBlockPos().add(1, 0, 0)).isOf(Blocks.BEDROCK) && Envy.mc.world.getBlockState(target.getBlockPos().add(-1, 0, 0)).isOf(Blocks.BEDROCK) && Envy.mc.world.getBlockState(target.getBlockPos().add(0, 0, 1)).isOf(Blocks.BEDROCK) && Envy.mc.world.getBlockState(target.getBlockPos().add(0, 0, -1)).isOf(Blocks.BEDROCK);
    }
