...
public class TargetUtils {
    ...
    public static boolean isPlayerNear(double range) {
        ...
        return get(entity -> {
            if (!(entity instanceof PlayerEntity) || entity == Envy.mc.player) return false;
            if (((PlayerEntity) entity).isDead() || ((PlayerEntity) entity).getHealth() <= 0) return false;
            if (Envy.mc.player.distanceTo(entity) > range) return false;
    ...
    public static boolean isBadTarget(PlayerEntity target, double range) {
        ...
        return Envy.mc.player.distanceTo(target) > range || !target.isAlive() || target.isDead() || target.getHealth() <= 0;
    }

    private static int sort(Entity e1, Entity e2, SortPriority priority) {
        return switch (priority) {
            case Lowest_Distance -> Double.compare(e1.distanceTo(Envy.mc.player), e2.distanceTo(Envy.mc.player));
            case Highest_Distance -> invertSort(Double.compare(e1.distanceTo(Envy.mc.player), e2.distanceTo(Envy.mc.player)));
    ...
    private static int sortAngle(Entity e1, Entity e2) {
        ...
        double e1yaw = Math.abs(Rotations.getYaw(e1) - Envy.mc.player.getYaw());
        double e2yaw = Math.abs(Rotations.getYaw(e2) - Envy.mc.player.getYaw());

        double e1pitch = Math.abs(Rotations.getPitch(e1) - Envy.mc.player.getPitch());
        double e2pitch = Math.abs(Rotations.getPitch(e2) - Envy.mc.player.getPitch());
