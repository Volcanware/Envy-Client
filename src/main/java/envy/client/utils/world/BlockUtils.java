...
public class BlockUtils {
    ...
    public static void init() {
        Envy.EVENT_BUS.subscribe(BlockUtils.class);
    }
    ...
    public static Block getBlock(BlockPos p) {
        ...
        return Envy.mc.world.getBlockState(p).getBlock();
    }

    public static BlockState getBlockState(BlockPos pos) {
        if (pos == null) return null;
        return Envy.mc.world.getBlockState(pos);
    }
    ...
    public static boolean isBlastResistant2(BlockPos pos) {
        return Envy.mc.world.getBlockState(pos).getBlock().getBlastResistance() >= 600;
    }
    ...
    public static Direction rayTraceCheck(BlockPos blockPos, boolean bl) {
        Vec3d vec3d = new Vec3d(Envy.mc.player.getX(), Envy.mc.player.getY() + Envy.mc.player.getEyeHeight(Envy.mc.player.getPose()), Envy.mc.player.getZ());
        for (Direction direction : Direction.values()) {
            RaycastContext raycastContext = new RaycastContext(vec3d, new Vec3d((double) blockPos.getX() + 0.5 + (double) direction.getVector().getX() * 0.5, (double) blockPos.getY() + 0.5 + (double) direction.getVector().getY() * 0.5, (double) blockPos.getZ() + 0.5 + (double) direction.getVector().getZ() * 0.5), RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, Envy.mc.player);
            BlockHitResult blockHitResult = Envy.mc.world.raycast(raycastContext);
    ...
    public static boolean place(BlockPos blockPos, FindItemResult findItemResult, boolean rotate, int rotationPriority, boolean swingHand, boolean checkEntities, boolean swapBack) {
        if (findItemResult.isOffhand()) return place(blockPos, Hand.OFF_HAND, Envy.mc.player.getInventory().selectedSlot, rotate, rotationPriority, swingHand, checkEntities, swapBack);
    ...
    private static void place(BlockHitResult blockHitResult, Hand hand, boolean swing) {
        ...
        ActionResult result = Envy.mc.interactionManager.interactBlock(Envy.mc.player, hand, blockHitResult);
    ...
    public static boolean canPlace(BlockPos blockPos, boolean checkEntities) {
        ...
        return !checkEntities || Envy.mc.world.canPlace(Envy.mc.world.getBlockState(blockPos), blockPos, ShapeContext.absent());
    }
    ...
    private static void placeEnhanced(int n, Vec3d vec3d, Hand hand, Direction direction, BlockPos blockPos, boolean bl, boolean bl2, boolean bl3) {
        ...
        if (bl2) Envy.mc.player.getInventory().selectedSlot = n;
        boolean bl4 = Envy.mc.player.input.sneaking;
        Envy.mc.player.input.sneaking = false;
        Envy.mc.interactionManager.interactBlock(Envy.mc.player, hand, new BlockHitResult(vec3d, direction, blockPos, false));
        if (bl) Envy.mc.player.swingHand(hand);
        else Envy.mc.getNetworkHandler().sendPacket(new HandSwingC2SPacket(hand));
        Envy.mc.player.input.sneaking = bl4;
        if (bl3) Envy.mc.player.getInventory().selectedSlot = n2;
    }

    public static boolean placePrinter(BlockPos blockPos, Direction direction, boolean swingHand) {
        if (Envy.mc.player == null) return false;
    ...
    private static void placePrinter(BlockHitResult blockHitResult, boolean swing) {
        if (Envy.mc.player == null || Envy.mc.interactionManager == null || Envy.mc.getNetworkHandler() == null) return;
        boolean wasSneaking = Envy.mc.player.input.sneaking;
        Envy.mc.player.input.sneaking = false;

        ActionResult result = Envy.mc.interactionManager.interactBlock(Envy.mc.player, Hand.MAIN_HAND, blockHitResult);

        if (result.shouldSwingHand()) {
            if (swing) Envy.mc.player.swingHand(Hand.MAIN_HAND);
            else Envy.mc.getNetworkHandler().sendPacket(new HandSwingC2SPacket(Hand.MAIN_HAND));
        }
    ...
    private static void onTickPost(TickEvent.Post event) {
        if (!breakingThisTick && breaking) {
            breaking = false;
            if (Envy.mc.interactionManager != null) Envy.mc.interactionManager.cancelBlockBreaking();
        }
    }

    /** Needs to be used in {@link TickEvent.Pre} */
    public static boolean breakBlock(BlockPos blockPos, boolean swing) {
        if (!canBreak(blockPos, Envy.mc.world.getBlockState(blockPos))) return false;
        ...
        if (Envy.mc.interactionManager.isBreakingBlock()) Envy.mc.interactionManager.updateBlockBreakingProgress(pos, Direction.UP);
        else Envy.mc.interactionManager.attackBlock(pos, Direction.UP);
    ...
    public static boolean canBreak(BlockPos blockPos, BlockState state) {
        if (!Envy.mc.player.isCreative() && state.getHardness(Envy.mc.world, blockPos) < 0) return false;
        return state.getOutlineShape(Envy.mc.world, blockPos) != VoxelShapes.empty();
    }

    public static boolean canBreak(BlockPos blockPos) {
        return canBreak(blockPos, Envy.mc.world.getBlockState(blockPos));
    }

    public static boolean canInstaBreak(BlockPos blockPos, BlockState state) {
        return Envy.mc.player.isCreative() || state.calcBlockBreakingDelta(Envy.mc.player, Envy.mc.world, blockPos) >= 1;
    }

    public static boolean canInstaBreak(BlockPos blockPos) {
        return canInstaBreak(blockPos, Envy.mc.world.getBlockState(blockPos));
    }
    ...
    public static MobSpawn isValidMobSpawn(BlockPos blockPos, boolean newMobSpawnLightLevel) {
        ...
        if (!(Envy.mc.world.getBlockState(blockPos).getBlock() instanceof AirBlock) || Envy.mc.world.getBlockState(blockPos.down()).getBlock() == Blocks.BEDROCK) return MobSpawn.Never;

        if (!topSurface(Envy.mc.world.getBlockState(blockPos.down()))) {
            if (Envy.mc.world.getBlockState(blockPos.down()).getCollisionShape(Envy.mc.world, blockPos.down()) != VoxelShapes.fullCube()) return MobSpawn.Never;
            if (Envy.mc.world.getBlockState(blockPos.down()).isTranslucent(Envy.mc.world, blockPos.down())) return MobSpawn.Never;
        }

        if (Envy.mc.world.getLightLevel(blockPos, 0) <= spawnLightLimit) return MobSpawn.Potential;
        else if (Envy.mc.world.getLightLevel(LightType.BLOCK, blockPos) <= spawnLightLimit) return MobSpawn.Always;
    ...
    public static boolean isExposed(BlockPos blockPos) {
        for (Direction direction : Direction.values()) {
            if (!Envy.mc.world.getBlockState(EXPOSED_POS.get().set(blockPos, direction)).isOpaque()) return true;
        }
