...
public class BEntityUtils {
    ...
    public static boolean isDeathPacket(PacketEvent.Receive event) {
        if (event.packet instanceof EntityStatusS2CPacket packet) {
            if (packet.getStatus() == 3) {
                deadEntity = packet.getEntity(Envy.mc.world);
    ...
    public static Direction rayTraceCheck(BlockPos pos, boolean forceReturn) {
        Vec3d eyesPos = new Vec3d(Envy.mc.player.getX(), Envy.mc.player.getY() + (double) Envy.mc.player.getEyeHeight(Envy.mc.player.getPose()), Envy.mc.player.getZ());
        Direction[] var3 = Direction.values();

        for (Direction direction : var3) {
            RaycastContext raycastContext = new RaycastContext(eyesPos, new Vec3d((double) pos.getX() + 0.5D + (double) direction.getVector().getX() * 0.5D, (double) pos.getY() + 0.5D + (double) direction.getVector().getY() * 0.5D, (double) pos.getZ() + 0.5D + (double) direction.getVector().getZ() * 0.5D), RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, Envy.mc.player);
    ...
    public static int getBlockBreakingSpeed(BlockState block, BlockPos pos, int slot) {
        PlayerEntity player = Envy.mc.player;
        ...
        float t = block.getHardness(Envy.mc.world, pos);
        if (t == -1.0F) {
            return 0;
        } else {
    ...
    public static Vec3d crystalEdgePos(EndCrystalEntity crystal) {
        ...
        return new Vec3d(
            crystalPos.x < Envy.mc.player.getX() ? crystalPos.add(Math.min(1, Envy.mc.player.getX() - crystalPos.x), 0, 0).x : crystalPos.x > Envy.mc.player.getX() ? crystalPos.add(Math.max(-1, Envy.mc.player.getX() - crystalPos.x), 0, 0).x : crystalPos.x,
            crystalPos.y < Envy.mc.player.getY() ? crystalPos.add(0, Math.min(1, Envy.mc.player.getY() - crystalPos.y), 0).y : crystalPos.y,
            crystalPos.z < Envy.mc.player.getZ() ? crystalPos.add(0, 0, Math.min(1, Envy.mc.player.getZ() - crystalPos.z)).z : crystalPos.z > Envy.mc.player.getZ() ? crystalPos.add(0, 0, Math.max(-1, Envy.mc.player.getZ() - crystalPos.z)).z : crystalPos.z);
    }

    public static boolean isBedrock(BlockPos pos) {
        return Envy.mc.world.getBlockState(pos).isOf(Blocks.BEDROCK);
    }
    ...
    public static boolean isBlastResistant(BlockPos pos, BlastResistantType type) {
        Block block = Envy.mc.world.getBlockState(pos).getBlock();
        switch (type) {
            case Any, Mineable -> {
    ...
    public static BlockPos getTargetBlock(PlayerEntity player) {
        ...
        List<BlockPos> myPositions = getSurroundBlocks(Envy.mc.player);
        ...
        for (BlockPos pos : positions) {
            ...
            if (Envy.mc.player.squaredDistanceTo(Utils.vec3d(pos)) < Envy.mc.player.squaredDistanceTo(Utils.vec3d(finalPos))) {
                finalPos = pos;
            }
        }
