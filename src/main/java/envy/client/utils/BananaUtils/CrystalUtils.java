...
import envy.client.systems.modules.combat.BananaBomber;
...
public class CrystalUtils {
    ...
    public static void attackCrystal(Entity entity) {
        // Attack
        Envy.mc.player.networkHandler.sendPacket(PlayerInteractEntityC2SPacket.attack(entity, Envy.mc.player.isSneaking()));

        if (BBomber.renderSwing.get()) Envy.mc.player.swingHand(Hand.MAIN_HAND);
        if (!BBomber.hideSwings.get()) Envy.mc.getNetworkHandler().sendPacket(new HandSwingC2SPacket(Hand.MAIN_HAND));
        BBomber.attacks++;
    ...
    public static boolean shouldIgnoreSelfPlaceDamage() {
        return (BBomber.PDamageIgnore.get() == BananaBomber.DamageIgnore.Always
            || (BBomber.PDamageIgnore.get() == BananaBomber.DamageIgnore.WhileSafe && (BEntityUtils.isSurrounded(Envy.mc.player, BEntityUtils.BlastResistantType.Any) || BEntityUtils.isBurrowed(Envy.mc.player, BEntityUtils.BlastResistantType.Any)))
    ...
    public static boolean shouldIgnoreSelfBreakDamage() {
        return (BBomber.BDamageIgnore.get() == BananaBomber.DamageIgnore.Always
            || (BBomber.BDamageIgnore.get() == BananaBomber.DamageIgnore.WhileSafe && (BEntityUtils.isSurrounded(Envy.mc.player, BEntityUtils.BlastResistantType.Any) || BEntityUtils.isBurrowed(Envy.mc.player, BEntityUtils.BlastResistantType.Any)))
