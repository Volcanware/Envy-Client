...
import envy.client.mixininterface.IExplosion;
import envy.client.mixininterface.IRaycastContext;
import envy.client.mixininterface.IVec3d;
import envy.client.systems.friends.Friends;
import envy.client.utils.PostInit;
...
public class BDamageUtils {
    ...
    public static void init() {
        Envy.EVENT_BUS.subscribe(BDamageUtils.class);
    }

    @EventHandler
    private static void onGameJoined(GameJoinedEvent event) {
        explosion = new Explosion(Envy.mc.world, null, 0, 0, 0, 6, false, Explosion.DestructionType.DESTROY);
        raycastContext = new RaycastContext(null, null, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.ANY, Envy.mc.player);
    }
    ...
    private static float getDamageForDifficulty(float damage) {
        return switch (Envy.mc.world.getDifficulty()) {
            case PEACEFUL -> 0;
    ...
    private static BlockHitResult raycast(RaycastContext context, boolean ignoreTerrain, boolean fullBlocks) {
        return BlockView.raycast(context.getStart(), context.getEnd(), context, (raycastContext, blockPos) -> {
            ...
            blockState = Envy.mc.world.getBlockState(blockPos);
            ...
            VoxelShape voxelShape = raycastContext.getBlockShape(blockState, Envy.mc.world, blockPos);
            BlockHitResult blockHitResult = Envy.mc.world.raycastBlock(vec3d, vec3d2, blockPos, voxelShape, blockState);
    ...
    public static float possibleHealthReductions(boolean crystals, double explosionRadius , boolean swords, float enemyDistance) {
        ...
        for (Entity entity : Envy.mc.world.getEntities()) {
            if (crystals) {
                ...
                if (entity instanceof EndCrystalEntity && damageTaken < crystalDamage(Envy.mc.player, entity.getPos(), explosionRadius)) {
                    damageTaken = crystalDamage(Envy.mc.player, entity.getPos(), explosionRadius);
                }
            }
            ...
            if (swords) {
                ...
                if (entity instanceof PlayerEntity && damageTaken < getSwordDamage((PlayerEntity) entity, true)) {
                    if (!Friends.get().isFriend((PlayerEntity) entity) && Envy.mc.player.getPos().distanceTo(entity.getPos()) < enemyDistance) {
        ...
        if (PlayerUtils.getDimension() != Dimension.Overworld) {
            for (BlockEntity blockEntity : Utils.blockEntities()) {
                ...
                if (blockEntity instanceof BedBlockEntity && damageTaken < bedDamage(Envy.mc.player, pos)) {
                    damageTaken = bedDamage(Envy.mc.player, pos);
                }
            }
        }
