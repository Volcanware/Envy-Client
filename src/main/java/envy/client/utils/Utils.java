...
import envy.client.mixin.ClientPlayNetworkHandlerAccessor;
...
import envy.client.systems.modules.render.BetterTooltips;
import envy.client.systems.modules.world.Timer;
import envy.client.utils.player.EChestMemory;
import envy.client.utils.render.PeekScreen;
import envy.client.utils.render.color.Color;
import envy.client.utils.world.BlockEntityIterator;
import envy.client.utils.world.ChunkIterator;
...
public class Utils {
    ...
    public static void init() {
        Envy.EVENT_BUS.subscribe(Utils.class);
    }

    @EventHandler
    private static void onick(TickEvent.Post event) {
        if (screenToOpen != null && Envy.mc.currentScreen == null) {
            Envy.mc.setScreen(screenToOpen);
            screenToOpen = null;
        }
    }

    public static boolean isDeveloper(String uuid) {
        return Envy.getDeveloperUUIDs().contains(uuid.replace("-", ""));
    }
    ...
    public static double getPlayerSpeed() {
        ...
        double tX = Math.abs(Envy.mc.player.getX() - Envy.mc.player.prevX);
        double tZ = Math.abs(Envy.mc.player.getZ() - Envy.mc.player.prevZ);
        double length = Math.sqrt(tX * tX + tZ * tZ);

        envy.client.systems.modules.world.Timer timer = Modules.get().get(envy.client.systems.modules.world.Timer.class);
    ...
    public static String getWorldTime() {
        if (Envy.mc.world == null) return "00:00:00";

        int ticks = (int) (Envy.mc.world.getTimeOfDay() % 24000);
        ticks += 6000;
    ...
    public static int getRenderDistance() {
        return Math.max(Envy.mc.options.getViewDistance().getValue(), ((ClientPlayNetworkHandlerAccessor) Envy.mc.getNetworkHandler()).getChunkLoadDistance());
    }

    public static int getWindowWidth() {
        return Envy.mc.getWindow().getFramebufferWidth();
    }

    public static int getWindowHeight() {
        return Envy.mc.getWindow().getFramebufferHeight();
    }

    public static void unscaledProjection() {
        RenderSystem.setProjectionMatrix(Matrix4f.projectionMatrix(0, Envy.mc.getWindow().getFramebufferWidth(), 0, Envy.mc.getWindow().getFramebufferHeight(), 1000, 3000));
        rendering3D = false;
    }

    public static void scaledProjection() {
        RenderSystem.setProjectionMatrix(Matrix4f.projectionMatrix(0, (float) (Envy.mc.getWindow().getFramebufferWidth() / Envy.mc.getWindow().getScaleFactor()), 0, (float) (Envy.mc.getWindow().getFramebufferHeight() / Envy.mc.getWindow().getScaleFactor()), 1000, 3000));
        rendering3D = true;
    }
    ...
    public static boolean openContainer(ItemStack itemStack, ItemStack[] contents, boolean pause) {
        if (hasItems(itemStack) || itemStack.getItem() == Items.ENDER_CHEST) {
            ...
            else Envy.mc.setScreen(new PeekScreen(itemStack, contents));
            return true;
        }
    ...
    public static Dimension getDimension() {
        switch (Envy.mc.world.getRegistryKey().getValue().getPath()) {
            case "the_nether" -> {
    ...
    public static Integer getMaxHeight() {
        return Envy.mc.world.getDimension().height() - Math.abs(getMinHeight()) - 1;
    }
    ...
    public static String getWorldName() {
        if (Envy.mc.isInSingleplayer()) {
            // Singleplayer
            File folder = ((MinecraftServerAccessor) Envy.mc.getServer()).getSession().getWorldDirectory(Envy.mc.world.getRegistryKey()).toFile();
            if (folder.toPath().relativize(Envy.mc.runDirectory.toPath()).getNameCount() != 2) folder = folder.getParentFile();

            return folder.getName();
        }

        if (Envy.mc.getCurrentServerEntry() != null) {
            // Multiplayer
            String name = Envy.mc.isConnectedToRealms() ? "realms" : Envy.mc.getCurrentServerEntry().address;
    ...
    public static Float getPlayerHealth() {
        if (Envy.mc.world == null) return 0f;
        if (Envy.mc.player == null) return 0f;
        if (Envy.mc.player.isDead()) return 0f;
        if (Envy.mc.player.isCreative()) return 0f;
        if (Envy.mc.player.isSpectator()) return 0f;
        return (float) Math.round(Envy.mc.player.getHealth() + Envy.mc.player.getAbsorptionAmount());
    }

    public static String nameToTitle(String name) {
        name = Arrays.stream(name.split("-")).map(StringUtils::capitalize).collect(Collectors.joining(" "));
        String title = name.replace("Mathax", "Envy");
    ...
    public static boolean canUpdate() {
        return Envy.mc != null && Envy.mc.world != null && Envy.mc.player != null;
    }

    public static boolean canOpenClickGUI() {
        if (canUpdate()) return Envy.mc.currentScreen == null;

        return Envy.mc.currentScreen instanceof TitleScreen || Envy.mc.currentScreen instanceof MultiplayerScreen || Envy.mc.currentScreen instanceof SelectWorldScreen;
    }
    ...
    public static void leftClick() {
        Envy.mc.options.attackKey.setPressed(true);
        ((MinecraftClientAccessor) Envy.mc).leftClick();
        Envy.mc.options.attackKey.setPressed(false);
    }

    public static void rightClick() {
        ((IMinecraftClient) Envy.mc).rightClick();
    }
    ...
    public static String getRandomPlayer() {
        ...
        int size = Envy.mc.player.networkHandler.getPlayerList().size();
        int r = random.nextInt(size);
        int c = 0;

        for (PlayerListEntry playerListEntry : Envy.mc.player.networkHandler.getPlayerList()) {
