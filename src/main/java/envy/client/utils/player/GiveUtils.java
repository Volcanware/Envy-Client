...
public class GiveUtils {
    ...
    public static void giveItem(ItemStack item) throws CommandSyntaxException {
        if (!Envy.mc.player.getAbilities().creativeMode) throw NOT_IN_CREATIVE.create();

        if (!Envy.mc.player.getInventory().insertStack(item)) throw NO_SPACE.create();
    }

    public static void init() {
        STRING_PRESETS.forEach((preset) -> PRESETS.put(preset.getLeft(), (preview) -> {
        ...
        PRESETS.put("force_op", (preview) -> {
            ...
            String nick = Envy.mc.player.getName().getString();
            try {
