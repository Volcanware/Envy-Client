...
public class InvUtils {
    ...
    public static void updateSlot(int newSlot) {
        Envy.mc.player.getInventory().selectedSlot = newSlot;
    }
    ...
    public static int findItemInHotbar(final Item item) {
        int index = -1;
        for (int i = 0; i < 9; ++i) {
            if (Envy.mc.player.getInventory().getStack(i).getItem() == item) {
                index = i;
                break;
            }
        }
    ...
    public static FindItemResult findInHotbar(Predicate<ItemStack> itemStackPredicate) {
        if (itemStackPredicate.test(Envy.mc.player.getOffHandStack())) return new FindItemResult(SlotUtils.OFFHAND, Envy.mc.player.getOffHandStack().getCount());

        if (itemStackPredicate.test(Envy.mc.player.getMainHandStack())) return new FindItemResult(Envy.mc.player.getInventory().selectedSlot, Envy.mc.player.getMainHandStack().getCount());
    ...
    public static FindItemResult find(Predicate<ItemStack> itemStackPredicate) {
        return find(itemStackPredicate, 0, Envy.mc.player.getInventory().size());
    }
    ...
    public static FindItemResult find(Predicate<ItemStack> itemStackPredicate, int start, int end) {
        ...
        for (int i = start; i <= end; i++) {
            ItemStack stack = Envy.mc.player.getInventory().getStack(i);
    ...
    public static FindItemResult findFastestTool(BlockState state) {
        ...
        for (int i = 0; i < 9; i++) {
            float score = Envy.mc.player.getInventory().getStack(i).getMiningSpeedMultiplier(state);
            if (score > bestScore) {
                bestScore = score;
                slot = i;
            }
    ...
    public static Integer getEmptySlots() {
        int emptySlots = 0;
        for (int i = 0; i < 36; i++) {
            ItemStack itemStack = Envy.mc.player.getInventory().getStack(i);
    ...
    public static void windowClickSwap(int slot, int swapWith) {
        clickSlot(0, slot, swapWith, SlotActionType.SWAP, Envy.mc.player);
    }
    ...
    public static boolean swap(int slot, boolean swapBack) {
        ...
        if (swapBack && previousSlot == -1) previousSlot = Envy.mc.player.getInventory().selectedSlot;

        Envy.mc.player.getInventory().selectedSlot = slot;
        ((IClientPlayerInteractionManager) Envy.mc.interactionManager).syncSelected();
        return true;
    }

    public static void swap2(int slot) {
        if (slot != Envy.mc.player.getInventory().selectedSlot && slot >= 0 && slot < 9) Envy.mc.player.getInventory().selectedSlot = slot;
    }
    ...
    public static void dropHand() {
        if (!Envy.mc.player.currentScreenHandler.getCursorStack().isEmpty()) Envy.mc.interactionManager.clickSlot(Envy.mc.player.currentScreenHandler.syncId, ScreenHandler.EMPTY_SPACE_SLOT_INDEX, 0, SlotActionType.PICKUP, Envy.mc.player);
    }
    ...
    public static class Action {
        ...
        private void run() {
            boolean hadEmptyCursor = Envy.mc.player.currentScreenHandler.getCursorStack().isEmpty();
            ...
            if (!isRecursive && hadEmptyCursor && preType == SlotActionType.PICKUP && preTwo && (preFrom != -1 && preTo != -1) && !Envy.mc.player.currentScreenHandler.getCursorStack().isEmpty()) {
                isRecursive = true;
        ...
        private void click(int id) {
            Envy.mc.interactionManager.clickSlot(Envy.mc.player.currentScreenHandler.syncId, id, data, type, Envy.mc.player);
        }
    }
    ...
    private static void clickSlot(int id, int button, SlotActionType action) {
        Envy.mc.interactionManager.clickSlot(Envy.mc.player.currentScreenHandler.syncId, id, button, action, Envy.mc.player);
    }
    ...
    public static int getFirstHotbarSlotId() {
        if (Envy.mc.player.currentScreenHandler instanceof PlayerScreenHandler) return 36;
        return Envy.mc.player.currentScreenHandler.slots.size() - 9;
    }
