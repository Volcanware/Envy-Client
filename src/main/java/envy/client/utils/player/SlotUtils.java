...
import envy.client.mixin.CreativeInventoryScreenAccessor;
import envy.client.mixin.HorseScreenHandlerAccessor;
...
public class SlotUtils {
    ...
    public static int indexToId(int i) {
        if (Envy.mc.player == null) return -1;
        ScreenHandler handler = Envy.mc.player.currentScreenHandler;
    ...
    private static int creativeInventory(int i) {
        if (!(Envy.mc.currentScreen instanceof CreativeInventoryScreen) || CreativeInventoryScreenAccessor.getSelectedTab() != ItemGroup.INVENTORY.getIndex()) return -1;
