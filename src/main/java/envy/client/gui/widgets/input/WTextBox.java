...
public abstract class WTextBox extends WWidget {
    ...
    public boolean onKeyPressed(int key, int mods) {
        ...
        if (control && key == GLFW_KEY_C) {
            if (cursor != selectionStart || cursor != selectionEnd) Envy.mc.keyboard.setClipboard(text.substring(selectionStart, selectionEnd));
            return true;
        } else if (control && key == GLFW_KEY_X) {
            if (cursor != selectionStart || cursor != selectionEnd) {
                Envy.mc.keyboard.setClipboard(text.substring(selectionStart, selectionEnd));
                clearSelection();
            }
    ...
    public boolean onKeyRepeated(int key, int mods) {
        ...
        if (control && key == GLFW_KEY_V) {
            ...
            String preText = text;
            String clipboard = Envy.mc.keyboard.getClipboard();
            int addedChars = 0;
